...
func CopyRegistryImageIfNeeded(envConfig *config.EnvConfig, applianceConfig *config.ApplianceConfig) (string, error) {
	...
	// Return the source registry URI so it can be used in REGISTRY_IMAGE env var
	// For OCP docker-registry images, this allows tagging with the original source reference
	return sourceRegistryUri, nil
}

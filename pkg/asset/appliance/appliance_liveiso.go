...
func (a *ApplianceLiveISO) Generate(_ context.Context, dependencies asset.Parents) error {
	...
	// Search for live ISO in cache dir
	if fileName := envConfig.FindInAssets(consts.ApplianceLiveIsoFileName); fileName != "" {
		logrus.Info("Configuring appliance live ISO")
		a.File = &asset.File{Filename: fileName}
	} else if err := a.buildLiveISO(envConfig, applianceConfig); err != nil {
		return err
	}

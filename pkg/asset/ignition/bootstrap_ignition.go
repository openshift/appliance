...
func (i *BootstrapIgnition) Generate(dependencies asset.Parents) error {
	...
	// Get cluster-resources generated by oc-mirror (CatalogSources, IDMS, and signature-configmap)
	// Note: signature-configmap.yaml is only generated for stable releases (x.y.z, x.y.z-ec.N, x.y.z-rc.N).
	// CI and nightly builds won't have this file.
	...
	logrus.Debugf("Loaded %d cluster-resources files from oc-mirror output", len(mirrorResources))
	for _, res := range mirrorResources {
		if strings.Contains(res.Filename, "signature-configmap") {
			logrus.Info("signature-configmap found in cluster-resources, will be applied to target cluster")
			break
		}
	}
...
func addExtraManifests(
	...
	for _, file := range fileList {
		...
		for n, manifest := range yamlList {
			...
			if strings.Contains(base, "signature-configmap") {
				logrus.Infof("Adding signature-configmap to extra manifests: %s", fileName)
			}
